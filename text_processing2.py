...
def digits_to_words(input_string):
    ...
    * 만약 인풋 스트링에서 숫자가 존재하지 않는 다면, 빈 문자열 (empty string)을 반환함
    ...
    word_list = [word for word in input_string if word.isdigit()]
    word_list = [num_list[int(num)] for num in word_list]
    digit_string = ' '.join(word_list)
    return digit_string
...
def to_camel_case(underscore_str):
    ...
    * 만약 어떤 변수 이름이 underscore로만 이루어 진다면, 빈 문자열만 반환해도 됨
    ...
    if len(word_list) == 1:
        ...
        word_list = [v.capitalize() if i != 0 else v.lower() for i,
                     v in enumerate(word_list)]
        camelcase_str = "".join(word_list)
    print(camelcase_str)
